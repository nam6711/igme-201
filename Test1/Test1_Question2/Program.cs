...
namespace Test1_Question2
{
    class Program
    {
        ...
        start:
            Console.WriteLine("Enter a sentence, we'll order the word's alphabetically!");
            ...
            // initialize the size of the unsorted array to sWord array length
            ...
            nUnsortedLength = 0;
            foreach (string sNextWord in sWord)
            {
                // still skip the blank strings
                if (sNextWord.Length == 0)
                {
                    continue;
                }

                // store the value into the array
                aUnsorted[nUnsortedLength] = sNextWord;
        ...
        static string FindLowestValue(string[] array)
        {
            ...
            foreach (string thisWord in array)
            {
                ...
                if (string.Compare(thisWord, returnVal) < 0)
                {
